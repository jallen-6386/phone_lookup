...
A free, privacy-focused reverse phone number lookup tool for the command line. Works fully offline for core data, and enriches results with free public APIs and community spam databases — no paid services required.
...
usage: phone_lookup [-h] [--batch FILE] [--format {table,json,csv}]
                    [--no-scrape] [--no-cache] [--clear-cache] [--region CC]
                    [number ...]
...
- Spam scraping (800notes, WhoCalledMe) adds ~2 seconds per lookup due to polite rate limiting between sites. Use `--no-scrape` if you only need carrier/line type data.
- International numbers outside North America receive offline data only (carrier, type, region, timezone). FCC, NANPA, and scraper providers are US/Canada only.
